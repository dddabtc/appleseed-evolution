...
from appleseed_evolution.runtime.proxy import run_server
from appleseed_evolution.workflow_state import build_resume_payload, build_workflow_state, render_resume_markdown


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="appleseed-evolution")
    subparsers = parser.add_subparsers(dest="command", required=True)

    init_parser = subparsers.add_parser("init", help="Write a sample config file.")
    init_parser.add_argument("path", nargs="?", default="appleseed.toml")
    init_parser.add_argument("--overwrite", action="store_true")

    skills_parser = subparsers.add_parser("skills", help="Inspect the loaded skill bank.")
    skills_parser.add_argument("--config", default="demo/appleseed.toml")
    ...
    route_parser.add_argument("--config", default="demo/appleseed.toml")
    ...
    feedback_parser.add_argument("--config", default="demo/appleseed.toml")
    ...
    ingest_parser.add_argument("--config", default="demo/appleseed.toml")
    ...
    openclaw_import_parser = subparsers.add_parser(
        "openclaw-import",
        help="Import a local OpenClaw operator session artifact or replayable handoff bundle into Appleseed runtime ledgers.",
    )
    openclaw_import_parser.add_argument("--config", default="demo/appleseed.toml")
    ...
    report_parser.add_argument("--config", default="demo/appleseed.toml")
    ...
    inspect_parser.add_argument("--config", default="demo/appleseed.toml")
    ...
    evolve_parser.add_argument("--config", default="demo/appleseed.toml")

    governance_parser = subparsers.add_parser(
        "governance",
        help="Inspect promotion readiness, risk, and rollback context for evolution proposals.",
    )
    governance_parser.add_argument("--config", default="demo/appleseed.toml")
    ...
    review_parser.add_argument("--config", default="demo/appleseed.toml")
    ...
    promote_parser.add_argument("--config", default="demo/appleseed.toml")
    ...
    resume_parser.add_argument("--config", default="demo/appleseed.toml")
    ...
    serve_parser.add_argument("--config", default="demo/appleseed.toml")
    return parser
...
def cmd_skills(args: argparse.Namespace) -> int:
    orchestrator = AppleseedOrchestrator.from_config_path(args.config)
...
def _persist_openclaw_import_chain(
    orchestrator: AppleseedOrchestrator,
    *,
    session_id: str,
...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    orchestrator = AppleseedOrchestrator(config)
    print(
        f"Serving Appleseed Evolution proxy on http://{config.runtime.host}:{config.runtime.port}",
        flush=True,
    )
